	...
	for  (count=1; count <= iBuffSize; count++)
	{
		...
		if (iBuff[count] != iBuff[compareto])
		{
			if (count - compareto > 3 && count - compareto < 254) //do something about more then 254 byte copies here 
			{
				...
				(*oBuff)[outbuffindex++] = (count - 1) - compareto;
				(*oBuff)[outbuffindex++] = iBuff[compareto];
				compareto = count;
			}/*
			else if(count - compareto > 3 && count - compareto > 254) //do something about more then 254 byte copies here 
			{
				int more = count - compareto;
				cout<<"count is, in >254 loop: "<< count - compareto <<endl;
				while(more > 254)
				{
				// compress it
				//  the first instance of the repeating byte is at: inBuff[compareto]
				//  the last instance of the repeating byte is at: inBuff[count - 1]
					...
					cout<<"(*oBuff)[outbuffindex++]= "<<(*oBuff)[outbuffindex++]<<endl;
...
bool dCompress::uData(BYTE* iBuff, int iBuffSize, BYTE** oBuff, int* oBuffSize)
{
	...
	*oBuff = new BYTE[bytesneed];
	...
	for (count=0; count < iBuffSize; count++)
	{
		//find the escape byte 0xFF
		if (ESCAPE_BYTE == iBuff[count]) 
		{
			if (((count+1) < iBuffSize) && (iBuff[count+1] != ESCAPE_BYTE))//if the next isnt a escape byte adn we are within the buff size
			{
				for (int count1=0; count1 <= iBuff[count+1]; count1++) //write out how man times the character should appear
				{
			...
			{
				(*oBuff)[obuffloc++] = iBuff[count++]; //two escape bytes in a row means copy one to the offbuff
			}
		}
		else
		{	//if we don't find 0xFF just copy to the outbuffer
